...
n_per_group <- 15    # Beobachtungen pro Gruppe
...
berechne_neg_grad <- function(data){
  neg_grad <- data$Y-mean(data$Y)
  neg_grad
  }


cv_stichprobe[[1]]$data_1_1$data$neg_grad <- berechne_neg_grad(cv_stichprobe[[1]]$data_1_1$data)
...
berechne_temp_residuen <- function(data, cart){
  ...
  data$temp_res <- data$Y - data$pred
  data
}
