...
pacman::p_load(
  ...
  foreach, randomForest, future.apply, lme4, tidyverse, caret,checkmate
)
```
...
# Gruppenspezifischer zufälliger Intercept
group_intercepts <- rnorm(n_groups, mean = 4, sd = 2)
random_intercepts <- group_intercepts[id]

# Neue lineare Effektgrößen
Y <- -2 +                   # anderer Grundwert
  0.5*X1 +                 # kleinerer Effekt von X1
  3*X2 +                   # viel größerer Effekt von X2
  0.8*X3 +                 # mäßiger Effekt von X3
  -0.5*X4 +                # negativer Effekt von X4
  1*X5 +                   # schwächerer Effekt von X5
  0.05*X6 +                # sehr kleiner Effekt von X6
  2*X7 +                   # stärkerer Effekt von X7
  -1*X8 +                  # X8 wirkt jetzt negativ
  1.5*X9 +                 # größerer Einfluss von X9
  0.005*X10 +              # fast kein Einfluss von X10
  random_intercepts +     
  rnorm(n_total, sd = 6)   # zufälliger Fehler etwas stärker
  ...
  for (i in 1:5) {
    ...
    while (iter <= itermax && improv_count < 5) {
      ...
      para_data$temp_res <- rep(NA,nrow(para_data))
        ...
        cart_modell <- schaetze_cart(para_data)
        # #print(cart_modell)
        para_data$temp_res <- berechne_temp_residuen(para_data, cart_modell, f_train)
        lmm_modell <- schaetze_lmm(para_data)
